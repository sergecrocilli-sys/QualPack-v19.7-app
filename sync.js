/* ================================================================
   ...
   V14c — corrections URL Supabase + IndexedDB
   ================================================================ */
...
async function syncPending(silent = false) {
  ...
  try {
    ...
    const pesees = Array.isArray(allPesees) ? allPesees.filter(r => r && (r.synced === false || r.synced === undefined)) : [];
    const dets = Array.isArray(allDets) ? allDets.filter(r => r && (r.synced === false || r.synced === undefined)) : [];
...
window.addEventListener('online', () => {
  ...
  syncPending(true).catch(err => console.warn('Auto sync online failed:', err));
});

if (navigator.onLine) {
  setTimeout(() => {
    syncPending(true).catch(err => console.warn('Auto sync startup failed:', err));
  }, 2000);
}
